...
初始值一般为 0 值，例如下面的类变量 value 被初始化为 0 而不是 123，在初始化的 `<clinit>` 中才会被设置为 123。
...
![j45_双委派机制.png](images/ClassLoader1.png)
...
- 高度的灵活性；
- 通过自定义类加载器可以实现热部署；（通过自定义的类加载方法，不使用双亲委派模式（java开头的类无法修改），从而实现热部署，即通过轮询的方式去每段时间主动加载某一个类，如果类的签名或者修改时间不同，则认为其被修改了，再次重新加载，否则不加载，tomcat默认支持热加载）
- 代码加密；
